...
- 可以通过 `--rollout-function-path` 参数，完全替换默认的 `generate_rollout`，只需要保证 `--rollout-function-path` 传入的函数签名满足：
  ...
  -  返回的 `Sample` 类型见 [slime/utils/types.py](https://github.com/vllm-project/vime/blob/main/slime/utils/types.py)，在实现时，需要保证
...
vLLM 参数引入 Vime 的方式可以参考 [slime/backends/vllm_utils/arguments.py](https://github.com/vllm-project/vime/blob/main/slime/backends/vllm_utils/arguments.py)。
