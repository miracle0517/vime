...
2. 将 `ROLLOUT_ARGS` 改名为了 `SFT_ARGS`，并配置为：
   ...
   Vime 中的 sft 实际上是复用了 Vime 的 custom rollout 功能，通过 `--rollout-function-path` 将数据生成部分从使用 vLLM 的 RL rollout，切换成了从文件中读取数据的 sft 版本，即 `slime.rollout.sft_rollout.generate_rollout`。
