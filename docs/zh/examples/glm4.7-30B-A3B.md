...
3. 在 vLLM 侧开启 MoE expert parallelism。GLM-4.7-Flash 是非 MLA 模型，vLLM 没有
   sglang `--enable-dp-attention` 的直接等价；这里用 attention 上 8 路 data parallel
   + expert 上 expert parallel：
