...
from vime.backends.sglang_utils.sglang_config import ModelConfig, ServerGroupConfig, SglangConfig
...
def _start_router(args, *, has_pd_disaggregation: bool = False, force_new: bool = False) -> tuple[str, int]:
    ...
    # MiniLB is PD-only in vllm-router; non-PD rollout needs the Rust Router (omit VIME_VLLM_ROUTER_USE_RUST).
    if has_pd_disaggregation and os.environ.get("VIME_VLLM_ROUTER_USE_RUST", "") != "1":
    ...
    if not process.is_alive():
        raise RuntimeError(
            ...
            "MiniLB is only valid with PD disaggregation. See vime.utils.http_utils run_router logs."
        )
