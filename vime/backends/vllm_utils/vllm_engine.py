...
def _forward_vllm_cli_args(args, cmd: list[str]) -> None:
    ...
    Uses the action table from ``vime.backends.vllm_utils.arguments.get_vllm_cli_action_table``
    ...
    from vime.backends.vllm_utils.arguments import get_vllm_cli_action_table
...
class VLLMEngine(RayActor):
    ...
    def __init__(
        ...
        # Vime runs one vLLM HTTP process per logical engine; multi-node worker rank is not used.
        self.node_rank = 0
    ...
    def _weight_transfer_http_timeout(self) -> float:
        if self._weight_transfer_http_timeout_s is None:
            self._weight_transfer_http_timeout_s = float(
                os.environ.get(
                    "VIME_VLLM_WEIGHT_TRANSFER_UPDATE_TIMEOUT_SEC",
                    os.environ.get("VIME_VLLM_WEIGHT_TRANSFER_HTTP_TIMEOUT_SEC", "900"),
                )
            )
