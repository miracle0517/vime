...
Mirrors vime/backends/sglang_utils/arguments.py:
...
SKIPPED_DESTS = [
    ...
    # subprocess when set; mirrors vime sglang_utils which exposes both via CLI.
...
def add_vllm_arguments(parser):
    ...
    # PD disaggregation / multi-group config — mirrors vime sglang_utils/arguments.py.
...
def vllm_parse_args():
    ...
    ``vime/utils/arguments.py`` is symmetric.
