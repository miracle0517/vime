...
def _model_device_type(model: torch.nn.Module) -> str | None:
...
def compute_dspark_loss(
    ...
    outputs = model(**batch, **train_kwargs)
    if not isinstance(outputs, (tuple, list)) or len(outputs) != 3:
        raise TypeError("Speculators DSpark forward must return (draft_tokens, loss, metrics)")
    _, loss, raw_metrics = outputs
    if not torch.is_tensor(loss) or not isinstance(raw_metrics, dict):
        raise TypeError("Speculators DSpark forward returned an invalid loss or metrics object")
