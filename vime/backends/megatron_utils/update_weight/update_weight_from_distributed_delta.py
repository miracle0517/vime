"""Delta weight sync (trainer / sender side).

Ported from slime ``slime/backends/megatron_utils/update_weight/update_weight_from_distributed_delta.py``.
The compute + encode core (bytewise diff, sparse position/value encode, pinned-CPU
snapshot, bucketing, async safetensors writer) is engine-agnostic and carried over
verbatim. The engine-facing parts are translated to vime's HTTP-backed vLLM engine
and vLLM-native NCCL weight transfer:

  * nccl transport: each bucket's (__positions__, __values__) is broadcast via
    ``NCCLWeightTransferEngine.trainer_send_weights`` on the ``PyNcclCommunicator``
    created by ``UpdateWeightFromDistributed.connect_rollout_engines`` (vime's
    ``trainer_init``). The matching receiver is the ``apply_delta_from_distributed``
    method on ``vLLMColocateWorkerExtension`` (invoked via the engine's
    ``update_weights_from_distributed_delta`` → ``POST /collective_rpc``). The
    engine RPC is fired with ``.remote()`` (non-blocking) so the HTTP POST blocks
    only the engine actor thread while this trainer broadcasts concurrently.
  * disk transport: each bucket is written as a safetensors file under
    ``--update-weight-delta-dir``; one ``update_weights_from_disk(load_format="delta",
    files=...)`` push per pass wakes the engines to read + apply.

For each sync, the sender bytewise-diffs the current weights against a pinned-CPU
snapshot of the last broadcast, packs the changed positions and values, and ships
only those. The receiver overwrites changed positions with the trainer's exact
bytes (no arithmetic), so the apply is lossless and there is no drift. The first
``update_weights`` call seeds the snapshot without contacting the rollout engines —
they're assumed to have loaded the same HF checkpoint at init.

Three encodings differ only in how positions are packed:
...
class UpdateWeightFromDistributedDelta(UpdateWeightFromDistributed):
    """
    Selective delta sync (non-colocate). ``--update-weight-transport`` picks the
    per-flush carrier: "nccl" broadcasts each bucket on the vLLM weight-transfer
    PyNCCL communicator; "disk" writes each bucket as a safetensors file under
    ``--update-weight-delta-dir`` and pushes once per pass.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # vime's base UpdateWeightFromDistributed has no metrics dict; the delta
        # path keeps its own for the per-sync density/byte log in _record_metrics.
        ...
        self._group_name = "vime-pp_0"  # overwritten on PP-src ranks in connect_rollout_engines
    ...
    def connect_rollout_engines(
        ...
        """
        NCCL transport: delegate to parent (creates the vLLM trainer-side
        PyNCCL communicator). Disk transport: just record the engines + PP-src
        flag (no NCCL group needed).
        """
    ...
    # -- HF chunk iterators (inlined; vime's base class uses a different shape) --
    ...
    def _iter_expert_chunks(
        ...
        callers restrict the iter to a subset (used by the delta sub-passes);
    ...
    def update_weights(self) -> None:
        ...
        Subsequent calls: pause → diff/encode → finalize → resume.
        """
    ...
    def _seed_snapshot(self) -> None:
        ...
        convert on PP-src ranks, D2H pinned copy). Cost is one full pass over params.
        """
    ...
    def _flush_bucket(self, bucket: DeltaBucket, pbar: tqdm | None) -> None:
        """
        NCCL: broadcast (__positions__, __values__) on the vLLM PyNCCL group with a
        DeltaSpec (+ shapes/dtype) carried as JSON via the engine collective_rpc.
        ...
        if self.transport == "nccl":
            self._send_delta_bucket_nccl(positions_gpu, values_gpu, params, checksum, pbar)
        else:  # disk
    ...
    def _send_delta_bucket_nccl(
        ...
        """
        Fire the engine collective_rpc (non-blocking ``.remote()``) so each worker
        enters ``apply_delta_from_distributed`` and posts a recv on the PyNCCL
        group, then broadcast (__positions__, __values__) from rank 0 via
        ``trainer_send_weights`` in the SAME order the receiver recvs them.

        The JSON manifest carries the DeltaSpec (encoding + per-param slices +
        checksum) plus the positions/values numel + values dtype so the receiver
        can allocate the recv buffers before the broadcast lands.
        """
        ...
        try:
            spec_dict = {
                ...
                "positions_numel": int(positions_gpu.numel()),
                "values_numel": int(values_gpu.numel()),
                "values_dtype": str(values_gpu.dtype).replace("torch.", ""),
            }
            delta_spec_json = json.dumps(spec_dict)
            refs = [
                engine.update_weights_from_distributed_delta.remote(
                    delta_spec_json=delta_spec_json,
                    weight_version=str(self.weight_version),
                )
                for engine in self.rollout_engines
            ]
            # Broadcast in the receiver's recv order: positions (uint8) then values.
            group = self._model_update_groups
            NCCLWeightTransferEngine.trainer_send_weights(
                iter([("__positions__", positions_gpu.contiguous())]),
                NCCLTrainerSendWeightsArgs(group=group, packed=False),
            )
            NCCLWeightTransferEngine.trainer_send_weights(
                iter([("__values__", values_gpu.contiguous())]),
                NCCLTrainerSendWeightsArgs(group=group, packed=False),
            )
            ray.get(refs)
        finally:
