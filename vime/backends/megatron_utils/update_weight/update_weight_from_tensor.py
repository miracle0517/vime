...
def _start_npu_weight_update_memory_monitor(worker) -> _NPUWeightUpdateMemoryMonitor:
    previous = getattr(worker, "_vime_weight_update_memory_monitor", None)
    if previous is not None:
        previous.stop()
    monitor = _NPUWeightUpdateMemoryMonitor(worker)
    worker._vime_weight_update_memory_monitor = monitor
...
def _patch_npu_colocate_worker() -> None:
    ...
    def _patched_start_weight_update(self, is_checkpoint_format: bool = True) -> None:
        monitor = _start_npu_weight_update_memory_monitor(self)
