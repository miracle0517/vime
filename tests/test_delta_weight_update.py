"""E2E smoke test for disk-backed delta weight updates (vime/vLLM).

Runs a tiny Qwen3.5-0.8B job so the first weight update seeds the delta
snapshot and the post-train update publishes sparse delta files, which the
rollout engine reads back via the delta disk-apply path. Ported from
THUDM/slime #1991; rollout side translated sglang->vLLM.
"""
...
def execute():
    with tempfile.TemporaryDirectory(prefix="vime_delta_weight_update_") as delta_dir:
        ...
        # Rollout engine = vLLM (slime used sglang). Non-colocate: actor 1 + rollout 3 = 4 GPU.
        vllm_args = (
